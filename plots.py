...
def blatt_02_P_01():
    ...
    fig.update_layout(autosize=False, width=500, height=500, margin=dict(l=65, r=50, b=65, t=90), hovermode=False)
...
def blatt_05_H_01_1(n):
    ...
    y_ = (2 * x_) * np.logical_and((x_ >= 0), (x_ <= 1 / 2)) + (2 * (1 - x_)) * np.logical_and((x_ >= 1 / 2), (x_ <= 1)) \
         + (-2 + 2 * x_) * np.logical_and((x_ >= 1), (x_ <= 3 / 2)) + (4 - 2 * x_) * np.logical_and((x_ >= 3 / 2),
                                                                                                    (x_ <= 2))
